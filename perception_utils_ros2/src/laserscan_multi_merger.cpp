/**
* @file laserscan_multi_merger.cpp
* @brief ROS 2 node that merges multiple laser scans into a fused scan and point cloud.
*
* This implementation subscribes to multiple `sensor_msgs::msg::LaserScan`
* topics, converts each scan to a point cloud, transforms those clouds into a
* shared destination frame, concatenates them, and republishes both the fused
* point cloud and a synthesized merged laser scan.
*/
...
#include "laserscan_multi_merger.hpp"
...
int main(int argc, char **argv)
{
	...
	rclcpp::spin(std::make_shared<perception_utils::LaserscanMerger>());
...
RCLCPP_COMPONENTS_REGISTER_NODE(perception_utils::LaserscanMerger);